...
const RutaCodigo = path.join(__dirname, "./VerificadorDeCodigo.json")
let Contenido
let ErrorGeneral
let Respuesta
let CodigoGuardado
...
function IniciarObjetoVerificadorDeCodigo() {
    ...
    if (!fs.existsSync(RutaCodigo)) {
        console.log("/nVerificadorDeCodigo: el archivo VerificadorDeCodigo.json fue eliminado")
        ErrorGeneral = true
    }
    // Paso -> leer el archivo json
        ...
        try {
            ...
            if (!Contenido) {
                console.log("/nVerificadorDeCodigo: el archivo VerificadorDeCodigo.json se encuentra completamente vacio")
                ErrorGeneral = true
            } else {
            ...
            console.log("/nVerificadorDeCodigo: el archivo VerificadorDeCodigo.json no se pudo leer")
            ErrorGeneral = true
        }
    }
