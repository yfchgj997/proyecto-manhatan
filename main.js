...
ipcMain.on("EAutenticarUsuario", (event, datos) => {
    ...
    if (usuarioEncontrado) {
        ...
        if (usuarioEncontrado.Estado !== "Activo") {
            console.log("Error: El usuario está inactivo");
            event.sender.send("ErrorLogin", "El usuario está inactivo y no puede acceder.");
            return;
        }
        ...
        event.sender.send("ErrorLogin", "Usuario o contraseña incorrectos.");
    }
});
...
// Evento -> guardar una nueva compra venta
ipcMain.on("EQuiereGuardarNuevoCompraVenta", (event, CompraVenta) => {

    // mensaje de flujo
    console.log("MENSAJE: guardando una nueva compra venta, estos son los datos:")
    console.log(CompraVenta)

    // Paso -> guardar en la base de datos
    respuesta = BDrespaldo.GuardarCompraVenta(CompraVenta)
    ...
    if (respuesta.error == false) {// se guardo sin errores
        ...
        let lista = BDrespaldo.TablaCV(new Date().toISOString().split('T')[0], new Date().toISOString().split('T')[0])
        event.sender.send("ActualizarTablaCompraVenta", lista.listaCV)
        ...
        event.sender.send("ModificarMensaje", {
            tipo: "MensajeMalo",
            texto: "Hubo un error al guardar la compra venta"
        });
    }
})
